...
envready install node --version 20
```

智能安装流程：
1. 检测是否已安装 → 跳过或提示升级
2. 选择最佳安装方式（优先版本管理器：fnm > nvm, pyenv, rustup...）
3. 自动处理依赖
4. 安装后验证
5. 失败时调用 AI 排障
...
|------|------|------|----------------|
| Homebrew | 生态丰富、社区活跃 | 仅 macOS/Linux、无 AI、无环境配置 | AI 排障 + 跨平台配置文件 |
| nix | 声明式、可复现 | 学习曲线陡峭 | 更友好的声明式 + AI 引导 |
| Ansible | 强大的自动化 | 太重、面向运维 | 面向个人开发者、轻量 |
